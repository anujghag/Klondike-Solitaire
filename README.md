...
* **Premium Theme Engine**: Choose between 9 aesthetic themes — each with its own card artwork or procedural faces, table felt, font family, suit color palette, and card-back design. Theme selection persists across sessions via `localStorage` and applies to **every game in the Pavilion**:
  * 🌌 **Abyssal Void** — Inspired by *Lord of the Mysteries*. Deep starfield backgrounds, fog overlays, and silver-on-dark card faces (52 painted cards)
  ...
  * ⚔️ **Maratha Glory** — 17th-century Maratha Empire figures as painted anime portraits (hearts & spades) plus ornate fort/weapon medallion art for diamonds & clubs, generated by `scripts/generateMarathaCards.mjs` and drop-in replaceable with final AI art (`IMAGE_PROMPTS_MARATHA.md`)
  * 🌊 **Ocean Depths** — bioluminescent deep-sea palette: coral, aqua, teal and abyssal blue suits with a wave-ripple card back
  * 🌿 **Jungle Canopy** — rainforest greens with tiger-orange and sun-yellow suits and a leaf-lattice card back
  * 🚀 **Deep Space** — neon nebula suits (fuchsia/cyan/indigo/violet) on near-black, starfield card back with a ringed planet
  * 💘 **Valentine Romance** — blush-pink light theme, rose-white card faces, heart-ribbon card back
  * 🐾 **Cutie Pets** — cartoon animal deck: 🐶 puppy hearts, 🐱 kitten diamonds, 🐼 panda clubs, 🐰 rabbit spades, guarded by a 🐯 tiger on the card back
